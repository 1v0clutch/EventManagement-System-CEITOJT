    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Event Details" maxWidth="max-w-7xl">
      <div ref={mainModalRef}>
          ...
          <div className="text-center">
            ...
            {!event.is_default_event && !event.is_personal && (
              <div className="flex items-center justify-center mt-2 gap-2 flex-wrap">
                {event.is_urgent && event.event_type === 'meeting' && (
        ...
        {/* Members dropdown button */}
          ...
          {/* Urgent banner — shown to everyone for urgent meetings */}
          {event.event_type === 'meeting' && event.is_urgent && (
            <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl p-3 sm:p-4">
              <svg className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
              <div>
                <p className="text-sm font-bold text-red-800">Urgent Announcement</p>
                <p className="text-xs text-red-700 mt-0.5">This is an urgent meeting notice. No response is required — attendance is expected.</p>
              </div>
            </div>
          )}

          {/* Accept / Decline — non-urgent meetings only */}
          {isInvited && event.event_type === 'meeting' && !event.is_urgent && (
